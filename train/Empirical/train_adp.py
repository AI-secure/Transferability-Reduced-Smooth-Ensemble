...
from train.Empirical.trainer import ADP_Trainer
...
# ADP Training params
parser.add_argument('--alpha', default=2.0, type=float, required=True)
parser.add_argument('--beta', default=0.5, type=float, required=True)
parser.add_argument('--adv-eps', default=0.2, type=float)
args = parser.parse_args()

if args.adv_training:
    mode = f"adv_{args.epsilon}_{args.num_steps}_{args.alpha}_{args.beta}"
else:
    mode = f"vanilla_{args.alpha}_{args.beta}"

args.outdir = f"/{args.dataset}/adp/{mode}/"
...
def main():
    ...
    for epoch in range(args.epochs):
        ADP_Trainer(args, train_loader, model, criterion, optimizer, epoch, device, writer)
